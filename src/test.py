...
def execute(input_file_path, output_file_path, algorithm):
    ...
    for param in param_values:
        model = Estimator(param, algorithm, attrib_columns, target)
    ...
    best_model = Estimator(best_param, algorithm, attrib_columns, target)
...
def process_directory(input_dir_path, output_dir_path, algorithm, option=None):
    ...
    for (dirpath, dirnames, filenames) in os.walk(input_dir_path):
        for filename in filenames:
            ...
            output_dir_path = os.path.join(output_dir_path, algorithm_name)
            if not os.path.exists(output_dir_path):
                os.makedirs(output_dir_path)
            output_file_name = os.path.splitext(os.path.basename(filename))[0] + '_results.txt'
            output_file_path = os.path.join(output_dir_path, output_file_name)
            
            execute(input_file_path, output_file_path, algorithm_name)
...
def main(input_file_path, output_dir_path, algorithm, option):
    ...
    execute(input_file_path, output_dir_path, algorithm_name)
